...
//서버가 응답은 두 부분으로 구분할 수 있습니다. 헤더와 바디. 바디는 제이슨(Json) 타입으로 응답하는 것인데 앞으로 진행하면서 다뤄볼 것입니다. 크게 어려운 부분은 없구요. 살펴봐야할 부분이 헤더입니다. 
//그 중에서도 헤더의 상태코드(Status code)를 잘 사용하면 다양한 정보를 담아서 클라이어트에게 전송할 수 있습니다. 응답 헤더의 상태코드는 세 자리 정수로 되어 있는데 크게 세 분류가 있습니다.
...
//4XX  4XX 상태코드는 클라이언트 요청이 잘못 되었을 경우 응답하는 코드입니다.
...
//let은 이전시간에 보았던 키워드죠? ES6에서 사용할 수 있는 블록 스코프의 변수 입니다. let 키워드를 이용해 users 변수를 선언했구요, 선언함과 동시에 배열 값을 변수에 할당하였습니다. 이 배열은 요소롤 자바스크립트 객체를 갖고 있는데요, 
//id와 name을 속성으로 갖는 객체입니다. 이것을 user 객체라고 하겠습니다. user 객체의 id는 객체의 식별자이고 name은 사용자의 이름을 저장합니다.

//라우팅 설정
//클라이언트는 users 변수에 저장된 사용자 데이터를 조회하도록 API를 요청할 할수 있는데요. 이 API를 "GET /users" 라고 이름 짓겠습니다. 익스프레스의 라이팅 함수 중 get() 함수를 이용하여 메쏘드가 GET임을 설정한뒤 첫번재 파라매터로 경로명인 
// "/users" 문자열을 넘겨주면 이 API에 대한 라우팅을 만들 수 있습니다.

app.get('/users', (req, res) => {
  // 여기에 라우팅 로직을 작성하면 됩니다.
});
//여기까지 작성하면 클라이언트가 "GET /usres" 로 요청할 경우 위에서 설정한 라우팅 함수가 동작할 것입니다. 이제 남은것은 라우팅 함수에서 users 변수의 값을 클라이언트에 보내 주기만 하면 됩니다. 
//라우팅 함수의 두 파라매터 중 두번 째 파라매터인 res가 응답 객체인데요, 응답객체의 함수중 json() 함수를 사용하면 제이슨 형식의 데이터를 클라이언트에게 보내줄 수 있답니다.
//JSON
...
그럼 클라이언트는 특정 사용자를 조회하는 요청을 서버에게 전달할 때 id도 함께 전달합니다. id가 1인 사용자 객체를 조회할 경우 서버는 users에 저장된 배열을 뒤져 id가 1인 객체를 클라이언트로 응답해 주면 되는 것이죠. 그럼 이번 API에 대한 라우팅 로직을 구현해 보겠습니다.
...
서버에서 라우팅로직을 이렇게 설정하면 "GET /users/1"에 대한 요청을 처리할 수 있을 것입니다. 만약 id가 2인 요청을 처리하려면 또 API를 만들어야 할까요? 이런 방식으로 말이죠.

app.get('/users/2', (req, res) =>  ... );
그렇게 할 수는 있습니다. 하지만 이것은 효율적인 방법이 아닙니다. id값이 예측할 수 없을 만큼 변경가능하기 때문입니다. 그래서 익스프레스에서는 이러한 동적인 값을 라우팅에 설정할 수 있는 방법을 제공합니다. 다음과 같이 코드를 변경해 보세요.

app.get('/users/:id', (req, res) => {
  console.log(req.params.id); // 사용자가 입력한 :id 값이 출력됨. (주의: 단 문자열 형식임 )
});
"/users/:id" 라는 문자열 형식으로 경로를 설정하면 요청 객체를 이용해 req.params.id로 클라이언트의 요청정보에 접근할 수 있습니다. 만약 사용자가 "GET /users/1" 로 요청한다면 req.params.id에는 "1"이라는 값이 들어가게 됩니다.
// 단, 주의할 것은 이 값이 숫자가 아닌 문자라는 것입니다. 클라이언트가 요청할때 서버로 오는 데이터는 전부 문자열 형식입니다. 기억하세요.

아이디로 유저 객체 검색
우선 클라이언트로부터 요청정보를 받는 것에는 성공했습니다. 이제는 id를 기반으로 서버에 있는 users배열을 뒤져 id가 일치하는 데이터를 찾아 요청한 클라이언트로 응답해주는 일만 남았습니다.
...
parserInt() 함수로 id 문자열 값을 정수형으로 변경했습니다. 왜냐구요? users 배열에 있는 user 객체를 검색할 건데 검색 기준이 되는 값이 id 입니다. 
//users에 있는 값은 숫자형 데이터인데 클라이언트가 요청한 데이터는 문자열이기 때문입니다. 정확한 검색을 위해 id를 문자열로 변경하는 것입니다.
//parserInt()는 문자열을 숫자로 변경하는 과정에서 에러가 발생하면 NaN을 되돌려 주게 되어있습니다. 이것은 명백히 요청한 클라이언트 쪽의 실수라고 할 수 있는데요 서버는 이러한 요청에 대해서도 클라이언트에게 적절한 응답을 해줘야합니다. 
//우리가 REST API에서 공부한게 있죠. 바로 상태 코드(Status code)입니다. 클라이언트의 잘못된 요청에 대해 400번 상태코드를 응답하기로 한것을 기억하시나요? 이것을 코드로 구현하면 아래와 같습니다.
...
parseInt() 결과가 NaN일 경우 id는 NaN 값이 들어가게 됩니다. if 조건문에서 NaN은 false와 동일합니다. 즉 if (!Nan)은 if (!false)와 동일한 조건문이 되는 결과인 셈이죠. id에 NaN이 할당되면 if 블록 안으로 진입하게 됩니다.

응답 객체인 res는 json() 함수 말고도 status() 함수를 제공합니다. 함수 이름으로도 알수 있듯이 상태코드를 설정하여 응답하는 기능입니다. 파라매터로 응답할 상태코드를 설정하면 됩니다.
...
클래스로 객체를 만들어 chris 변수에 저장했습니다. 그리고 chris 객체의 greeting() 함수를 호출했구요. 
greeting() 함수는 콘솔에 인사 문자열을 출력하고 나서 this를 리턴합니다. this는 chris 객체를 의미하죠. 그래서 곧바로 introduce() 메소드를 호출하는 것입니다. 
//이것을 함수 체이닝이라고 하는데 함수를 체인처럼 연결해서 사용하는 모습을 연상하시면 이해될 것입니다.

//클라이언트로 응답
//다시 우리 코드로 돌아옵니다. 클라이언트가 요청한 id 정보를 검증하였습니다. 이제 남은 것은 users 배열에서 id와 일치하는 user 객체를 찾는 일입니다. 자바스크립트 배열 메소드 중 filter() 함수를 사용해 보겠습니다.
...
users.filter() 함수는 파라매터로 users 배열에 있는 요소를 순서대로 반환합니다. users.filter(user => ) 코드에서 처음 user의 값은 users 배열의 첫번째 요소인 {id: 1, name: 'alice'} 객체가 됩니다. 이 객체의 id 값과 요청한 id값을 비교해서 같으면 참을 다르면 거짓을 반환하는 것이 다음 코드입니다.

참을 반환하면 현재 user 객체를 새로운 배열에 추가합니다. 만약 그렇지 않을 경우에는 무시하구요. 그러면 위 코드 결과 user에는 어떤 값이 들어갈까요? 바로 다음과 같은 배열이 담기게 됩니다.
...
console.log(user); // {id: 1, name: 'alice'}
404 에러
만약에 filter() 함수로 검색에 실패할 경우는 어떻게 될까요? filter() 함수는 빈 배열([ ]) 을 반환하게 될 것입니다. 그리고 빈 배열의 0번 인덱스에 접근하게되면 undefined 값이 리턴됩니다. 결국 user 변수에는 undefined 값이 저장되는 것이죠. 이러한 경우 REST API 규칙에 의해 404 상태코드를 클라이언트에게 알려 줘야 합니다. 왜냐하면 id에 해당하는 유저 데이터가 없기 때문입니다. 다음 코드를 추가하세요.
...
{"id":1,"name":"chris"}%
전체 배열이 응답되는 대신에 하나의 객체만 응답되었죠? 그리고 그 객체의 id가 1인 것을 보니 요청한 데이터가 제대로 응답되는 것 같습니다.
...
라우팅 먼저 설정합시다. 그동안 익스프레스 객체 app의 get() 함수만 사용했는데 이것은 조회 API의 메소드가 GET 이었기 때문입니다. 삭제 API의 메소드인 DELETE을 설정하려면 delete() 함수를 사용해야겠지요. delete() 함수의 파라메터도 get() 함수와 같습니다. 첫번째 파라매터로 설정할 경로를 문자열로 넘겨줍니다.

app.delete('/users/:id', (req, res) => /* ... );
파라메터로 받은 id값에 대한 처리도 해야하는데 이미 "GET /users/:id" API를 만들면서 사용했습니다. 동일한 코드를 사용하면 됩니다.
...
삭제 로직 구현
잠깐 삭제 API의 로직을 생각해 봅시다. 먼저 유저 데이터를 담고 있는 users 배열에서 id에 해당하는 객체의 위치를 찾아야 되겠죠. 그리고 찾은 위치의 요소를 배열에서 제거하면 될 것입니다. 자바스크립트 배열 메소드 중에는 이러한 기능을 할수 있는 함수가 있는데 다음 두 가지 함수를 사용할 겁니다.

findIndex()
splice()
배열에서 삭제할 유저 찾기
findIndex()는 배열을 순회하면서 어떤 기준에 맞는 요소의 인덱스를 찾는데 사용합니다. users에서 요청한 id에 해당하는 객체가 있는 인덱스를 먼저 찾아야겠지요.

const userIdx = users.findIndex(user => {
  return user.id === id;
});
앞에서 먼저 보았던 배열 메소드 filter()와 매우 유사하게 동작합니다. findIndex()는 배열의 각 요소를 순차적으로 돌면서 계산합니다. users.findIndex(user, => ) 구문의 경우 처음 user는 users 배열의 첫 요소가 반환되어 나옵니다. 이 user에 대해 id값을 비교한 뒤 참을 반환하면 users 배열에서의 user가 있는 배열 인덱스가 반환됩니다. 결국 이 인덱스 정수값은 userIdx 상수에 저장되는 것이지요. 만약 id 비교 결과 거짓이 리턴되면 배열의 다음 요소가 user 변수에 할당되어 넘어옵니다.

만약 findIndex() 함수의 id 비교문이 전부 false를 반환하면 어떤 인덱스 값을 반환하게 될까요? 이 함수는 그거한 경우는 함수 동작의 실패로 판단하고 -1값을 반환합니다. 우리는 userIdx의 값이 -1일 경우 없는 유저라고 판단할수 있겠죠. 그럼 생각나는 것이 있죠? 바로 클라이언트에서 없는 유저라는 뜻의 404 상태코드를 응답해 주는 것입니다. 다음 코드를 보면 이해하시겠죠?
...
* Connection #0 to host 127.0.0.1 left intact
{"error":"Unknown user"}%
id가 1인 유저를 조회했을 때 404 상태코드가 응답되었습니다. 그리고 바디에는 애러 문자열이 들어있네요. 전체 데이터를 조회해 볼까요?
...
좀 상세하게 들여다 보죠. https 부분을 프로토콜이라고 합니다. 그 뒤에 www.google.co.kr 부분을 도메인이라고 합니다. 그 뒤에 따라오는 /#newwindow=1 부분을 경로라고 합니다. 마지막 으로 &q=chris 부분을 "쿼리 문자열""이라고 합니다. 
//브라우져가 서버로 뭐가 요청할때 q=chris라는 형식으로 요청하는 것입니다.
...
//body-parser	익스프레스에 요청 바디의 데이터에 접근하기 위해서 body-parser라는 패키지를 추가해야 합니다. body-parser를 프로젝트에 추가합니다.

npm i body-parser --save
바디 파서는 미들웨어입니다. 따라서 app.js에 있는 익스프레스 객체에 이 미들웨어를 추가해서 사용할 수 있죠. 미들웨어를 추가할 때 사용하는 익스프레스 객체의 함수가 뭐라고 했는지 기억 나시나요? 바로 use() 함수입니다. 바디 파서를 use() 함수로 추가해 봅시다.

app.use(bodyParser.json());
app.use(bodyParser.urlencoded({ extended: true }));
이 설정 코드에 대한 설명을 별도로 하지 않겠습니다. 궁금하신 분들은 여기를 읽어 보세요.
...
name 상수에 req.body.name을 할당하는 코드를 작성했습니다. 하지만 만약에 이 값이 undefined가 될수 있는데요, 클라이언트가 요청시 name 값을 입력하지 않은 경우입니다. 그러한 경우에는 빈 문자열을 name 상수에 넣도록 했습니다.
...
req.body.name의 값이 없을 경우 상수 name에는 길이가 0인 문자열("")이 들어가게 됩니다. 이것은 if (!name.length) 조건문을 통과하여 if 블록에 집입하게 되는 것이죠. 결국 요청한 클라이언트에게는 400 응답코드와 에러 메세지를 던져 줍니다.

새로운 아이디 만들기
요청한 파라매터 name이 제대로 입력 되었다면 다음 할일은 새로운 유저 객체를 만드는 것입니다. 유저객체는 id와 name으로 구성되어 있는데요 방금 name은 요청 바디를 통해 얻었습니다. 남은 것이 id 인데요 이것을 얻으려면 기존에 있는 id와 중복되지 않은 값을 사용해야 합니다. 저는 기존에 있는 id중 가장 큰 값보다 1 더큰 값을 id로 만들 생각이에요. 이번에도 자바스크립트 배열에서 제공하는 메소드를 사용할 것입니다. 바로 reduce() 함수입니다.

reduce() 함수는 배열의 각 요소를 순회하면서 어떤 누적데이터를 만들어 내는 기능을 합니다. 
직접 사용하는 것을 살펴 보면서 이해해 보도록 하죠.

const id = users.reduce((maxId, user) => {
   return user.id > maxId ? user.id : maxId
 }, 0);
reduce() 함수는 첫번째 함수를 파라매터로 넘겨주는데 이 함수는 두 개의 파라매터를 갖습니다. 
번째 파라매터로 maxId라고 이름한 것은 우리가 reduce() 함수가 종료될 때 얻게될 값을 저장하고 있습니다. 
두번째 파라매터는 배열 users의 각 요소를 순서대로 반환해 주는 유저 객체입니다.
 그리고 reduce() 함수의 두번째 파라매터로 0을 넘겨줬는데 이는 maxId의 초기 값이죠.

예를 들어 users.reduce(maxId, user) => { }, 0); 구문이 처음 실행될때 maxId 값은 0이 되고 user는 users 배열의 첫번째 요소가 됩니다.
...
결국 id 상수에는 users 배열에 있는 id 값 중 가장 큰 값이 들어 들어갈 것입니다. 우리는 새로운 id를 만들 것이기 때문에 이 값보다 1 큰값으로 id 상수를 만들겠습니다. reduce() 함수 바로뒤에 + 1 코드 보이시죠?
...
익스프레스 모듈의 Router 클래스로 객체를 만들어 router 상수에 할당합니다. 그리고 router 객체에서 제공하는 get(), delete(), post() 따위의 함수로 라우팅 로직을 구현합니다. 이것은 우리가 익스프레스 객체 app을 이용한 것과 매우 똑같습니다. 마지막으로 moduel.epxorts를 이용해 노드 모듈로 만들었습니다.

User 라우팅 모듈 만들기
api/users/index.js에 라우팅 모듈을 만들어 보겠습니다. app.js에 있는 라우팅 코드 부분을 이쪽으로 옮깁니다. 그리고 app 상수를 모두 router 상수로 변경합니다. 바뀐 것은 이것 뿐입니다.
...
module.exports = router;
User 라우팅 모돌 사용하기
app.js에는 user 라우팅 코드가 없어졌습니다. 모두 api/users/index.js 파일로 모듈화 되어 이동되었기 때문이죠. 이제는 이 모듈을 app.js에서 불러와 사용해야 합니다. 자 여기서 중요한 것이 있습니다. 익스프레스 Router 클래스로 만든 User 모듈은 익스프레스의 미들웨어가 된 것입니다. 그렇기 때문에 익스프레스 객체 app은 use() 함수로 이 미들웨어를 사용할 수 있게 되었습니다. 아래 app.js 파일을 다시 보세요.
...
그러면 다시 api/user 모듈로 이동해 봅시다. /users로 들어오 요청에 대해 이제는 경로 앞부분의 "/users"는 제외한 하위 경로로 설정해 주어야 합니다. api/user/index.js 파일을 변경하면 다음과 같습니다.
...
라우팅 컨트롤러 만들기
app.js에서 api/user/index.js 로 코드를 이동하면서 코드의 가독성을 높였습니다. app.js에는 익스프레스 설정에 관련된 코드만 있고 api 폴더에는 각 리소스 별로 (여기에서는 user 뿐이지만) 라우팅 로직이 들어있기 때문입니다.
